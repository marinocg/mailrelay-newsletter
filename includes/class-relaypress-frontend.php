...
final class RelayPress_Frontend {
	...
	public static function register_shortcode(): void {
		add_shortcode( 'relaypress_newsletter', array( __CLASS__, 'shortcode' ) );
	}
	...
	public static function shortcode( $atts = array() ): string {
		...
		$form    = RelayPress_Form_Use_Cases::get_form_for_render( $form_id );
		if ( ! $form ) {
	...
	private static function render_missing_form_notice(): string {
		if ( function_exists( 'current_user_can' ) && current_user_can( 'manage_options' ) ) {
			...
			$message    = sprintf(
				'%s <a href="%s">%s</a>.',
				esc_html__( 'RelayPress is active, but no forms are published yet.', 'relaypress-newsletter' ),
				esc_url( $create_url ),
				esc_html__( 'Create a form', 'relaypress-newsletter' )
			);

			return '<div class="relaypress-empty-form-notice">' . wp_kses_post( $message ) . '</div>';
		}
		return '';
	}
	...
	private static function ensure_assets( bool $needs_extensions, array $config, int $form_id ): void {
		...
		add_action(
			...
			<script>
				(function() {
					var relaypressAjaxFallbackMessage = <?php echo wp_json_encode( self::$ajax_error_msg ); ?>;
	...
	private static function render_form( array $config, string $extra_class, int $form_id ): string {
		$email_placeholder = (string) ( $config['fields']['email']['placeholder'] ?? $config['basics']['email_placeholder'] ?? __( 'Email...', 'relaypress-newsletter' ) );
		$title             = (string) ( $config['basics']['title'] ?? '' );
		$desc              = (string) ( $config['basics']['description'] ?? '' );
		$submit            = (string) ( $config['basics']['submit_label'] ?? __( 'Subscribe', 'relaypress-newsletter' ) );
		$group_ids         = (string) ( $config['destination']['group_ids'] ?? '' );
		$privacy_url       = (string) ( $config['consent']['privacy_url'] ?? '' );
		$consent_label     = (string) ( $config['consent']['label'] ?? __( 'I accept the privacy policy', 'relaypress-newsletter' ) );
		...
		$msg_html    = '';
		$form_marker = isset( $_GET['relaypress_form_id'] ) ? absint( wp_unslash( $_GET['relaypress_form_id'] ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( $form_marker && $form_marker !== $form_id ) {
			$msg_html = '';
		} elseif ( isset( $_GET['relaypress_status'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$st = sanitize_text_field( (string) wp_unslash( $_GET['relaypress_status'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( 'ok' === $st ) {
				$msg_html = sprintf(
					'<p class="relaypress-msg relaypress-ok">%s</p>',
