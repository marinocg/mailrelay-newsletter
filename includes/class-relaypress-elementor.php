 ...
 * Elementor integration.
 */
final class RelayPress_Elementor {
	...
	public static function register_elementor_category( $elements_manager ): void {
		...
		$elements_manager->add_category(
			'relaypress',
			array(
				'title' => __( 'Uve Mailrelay', 'relaypress-newsletter' ),
				'icon'  => 'fa fa-envelope',
			)
		);
	}
	...
	public static function register_elementor_widget( $widgets_manager ): void {
		...
		if ( ! class_exists( 'RelayPress_Elementor_Newsletter_Widget' ) && class_exists( '\Elementor\Widget_Base' ) ) {
			$path = __DIR__ . '/class-relaypress-elementor-newsletter-widget.php';
			if ( file_exists( $path ) ) {
				require_once $path;
			}
		}

		if ( ! class_exists( 'RelayPress_Elementor_Newsletter_Widget' ) ) {
			return;
		}

		$widget = new RelayPress_Elementor_Newsletter_Widget();
	...
	public static function register_form_controls( $widget, array $opts ): void {
		...
		if ( empty( $forms ) ) {
			...
			$message    = sprintf(
				...
				__( 'No forms yet. <a href="%s" target="_blank" rel="noopener noreferrer">Create your first form</a>.', 'relaypress-newsletter' ),
				esc_url( $create_url )
			);
			$widget->add_control(
				'form_empty_notice',
