 ...
 */
class RelayPress_Single_Form_Repository implements RelayPress_Form_Repository_Interface {
	...
	public function create( string $name, array $config, string $status ): ?RelayPress_Form {
		...
		update_post_meta( $post_id, RelayPress_Form::META_CONFIG, $config );
		update_post_meta( $post_id, RelayPress_Form::META_VERSION, RelayPress_Form::CONFIG_VERSION );

		$post = get_post( $post_id );
		return $post instanceof WP_Post ? RelayPress_Form::from_post( $post ) : null;
	}
	 ...
	 * @return RelayPress_Form|null
	 */
	private function get_primary_by_statuses( array $statuses ): ?RelayPress_Form {
	...
	protected function build_forms_from_posts( array $posts ): array {
		...
		foreach ( $posts as $post ) {
			if ( $post instanceof WP_Post ) {
				$forms[] = RelayPress_Form::from_post( $post );
			}
		}
		return $forms;
	}
