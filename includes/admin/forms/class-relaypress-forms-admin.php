 ...
 * Forms admin screens.
 */
final class RelayPress_Forms_Admin {
	/**
	...
	public static function admin_init(): void {
		...
		if ( isset( $_GET['relaypress_refresh_groups'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		if ( isset( $_POST['relaypress_form_save'] ) ) {
			self::handle_save();
		}

		$page = sanitize_text_field( (string) wp_unslash( $_GET['page'] ?? '' ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( 'relaypress-newsletter-forms' === $page && isset( $_GET['action'], $_GET['form_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$nonce = sanitize_text_field( (string) wp_unslash( $_GET['_wpnonce'] ?? '' ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( '' === $nonce || ! wp_verify_nonce( $nonce, 'relaypress_forms_action' ) ) {
				return;
			}
	...
	public static function admin_enqueue( string $hook_suffix ): void {
		if ( false === strpos( $hook_suffix, 'relaypress-newsletter-forms' ) ) {
			return;
		}
		...
		wp_enqueue_style( 'relaypress-admin-forms', $css_src, array(), RelayPress_Newsletter::VERSION );
		wp_enqueue_script( 'relaypress-admin-forms', $js_src, array(), RelayPress_Newsletter::VERSION, true );

		$show_upgrade = (bool) apply_filters( 'relaypress_show_upgrade_ui', true );
		if ( $show_upgrade ) {
			$up_js = plugins_url( 'assets/admin-upgrade.js', $base );
			wp_enqueue_script( 'relaypress-admin-upgrade', $up_js, array(), RelayPress_Newsletter::VERSION, true );
		}
	}
	...
	public static function render_forms_page(): void {
		...
		$renderer = apply_filters( 'relaypress_forms_admin_renderer', null );
		...
		if ( in_array( $action, array( 'edit', 'new' ), true ) ) {
			RelayPress_Forms_Editor_Page::render();
			return;
		}

		RelayPress_Forms_List_Page::render();
	}
	...
	private static function forms_menu_label(): string {
		$default = __( 'Forms', 'relaypress-newsletter' );
		return (string) apply_filters( 'relaypress_forms_menu_label', $default );
	}
	...
	private static function handle_save(): void {
		check_admin_referer( 'relaypress_form_save' );
		...
		$config                   = RelayPress_Form_Config::sanitize( $raw_config, $defaults );
		$config['basics']['name'] = $name;

		$form = $form_id
			? RelayPress_Form_Use_Cases::update_form( $form_id, $name, $config, $status )
			: RelayPress_Form_Use_Cases::create_form( $name, $config, $status );
	...
	private static function handle_row_action( string $action, int $form_id ): void {
		...
		check_admin_referer( 'relaypress_forms_action' );

		if ( 'duplicate' === $action ) {
			$form = RelayPress_Form_Use_Cases::duplicate_form( $form_id );
			self::redirect_with_notice( $form ? 'duplicated' : 'error' );
		}

		if ( 'trash' === $action ) {
			$ok = RelayPress_Form_Use_Cases::trash_form( $form_id );
